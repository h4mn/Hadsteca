...
call:getPath 2
@set PATH_WORKSPACE=%PATH_BASES%\%getPath_return%
call:getPath 3
...
rem @set EXE_ISQL=%PATH_FB3%\ISQL.EXE
@set EXE_ISQL="C:\Program Files\Firebird\Firebird_3_0\isql.exe"

@set MGS_OK=Base nova criada
@set MGS_ERRODEFAULT=Nao e possivel criar uma base nova
...
@if %COMPUTERNAME% EQU %VAR_PC_AUTORIZADO% (
	...
	@set MSG_ERRO=Este script nao pode ser executado fora do ambiente
	@goto:ERROR
)
...
::-----
:DadosOrigemExiste
@set MSG_ERRO=O arquivo %FILE_DADOSORIGEM% ja existe
goto:ERROR
:ERROR
