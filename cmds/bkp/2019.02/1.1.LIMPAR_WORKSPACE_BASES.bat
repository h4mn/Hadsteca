...
@set PATH_BACKUP=Z:\BACKUP\_BASES
@set PATH_FONTES=Z:\BACKUP\_FONTES
...
@set MSG_ERRORDEFAULT=Não foi possível limpar o workspace
	...
	@for /f "tokens=*" %%d in ('dir /o:-d /t:c /b /s /a:d "%PATH_WORKSPACE%\DESTINO"') do (
		...
		@echo - Compactando IMP_ID.ZIP...
			...
			@echo -- "!PATH_DOCUMENTOS!\*.*"
				...
				if %ERRORLEVEL% NEQ 0 goto:ERROR
			rem Não tem mais a pasta PRINTS
