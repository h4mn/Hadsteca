...
@if %COMPUTERNAME% EQU %VAR_PC_AUTORIZADO% (
	...
	if exist "%FILE_BASELIMPAEXE_DESTINO%" (
		rem call:Debug "Limpa j� existe"
		@pushd "%PATH_DESTINO%"
		call:extraiLimpa "!FILE_BASELIMPAEXE_DESTINO!" %FILE_LIMPA_EXE%
		@popd
	) else (
		@echo Copiando %FILE_LIMPA_EXE%...
...
@echo on
@echo %~1
@echo.&echo Debugging... &pause>nul
goto:END

::FUNCOES
::-------
:extraiLimpa FullPath FileName
@echo Extraindo %~2...
@"%~1" -s >nul
goto:eof
